#!/usr/bin/env python3
"""T2 port of plot_hypodd_t1_speed_map.py: same view/crop/smoothing as
plot_hypodd_t2_shear_margin_map.py, showing ice surface speed instead of
strain rate, for side-by-side comparison and to get an actual m/yr number
for the array.

Usage:
    python full_catalog_pipeline/plot_hypodd_t2_speed_map.py
"""
...
from plot_hypodd_t2_shear_margin_map import (
    COLOR_PRE, COLOR_POST, COLOR_STATION, OUT_DIR, PAD_KM, SMOOTH_SIGMA_PX, SMOOTH_KM,
    VX_CACHE, VX_URL, VY_CACHE, VY_URL,
    fetch_component_window, load_events, load_stations,
)

OUT_PNG = f"{OUT_DIR}/hypodd_t2_reloc_speed_map.png"
...
def main():
    ...
    # point sample at the array centroid itself -- the single-number "T2 speed"
    center_row = int((extent_km[3] - 0) / (extent_km[3] - extent_km[2]) * speed.shape[0])
    center_col = int((0 - extent_km[0]) / (extent_km[1] - extent_km[0]) * speed.shape[1])
    centroid_speed = speed[max(center_row - 2, 0):center_row + 3,
                            max(center_col - 2, 0):center_col + 3].mean()
    print(f"speed over displayed crop: {visible.shape}, vmin={vmin:.2f} "
          f"vmax(true peak)={vmax:.2f} m/yr, median={np.median(visible_positive):.2f} m/yr")
    print(f"speed at T2 array centroid (5x5 px mean, {SMOOTH_KM}km-smoothed): {centroid_speed:.2f} m/yr")
    ...
    y_coords = np.linspace(extent_km[3], extent_km[2], speed.shape[0])
    ...
    ax.set_title("T2 relocations vs. ITS_LIVE ice speed (same view/smoothing as the\n"
