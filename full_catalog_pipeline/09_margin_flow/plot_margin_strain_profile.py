#!/usr/bin/env python3
"""Seismicity and surface strain rate as a function of across-margin distance (T1 or T2).

Companion to plot_margin_oriented_sections.py, which establishes the margin frame from
...
Usage:
    python full_catalog_pipeline/plot_margin_strain_profile.py --array T1
    ICEQUAKE_RELOC=hypodd_vels1d_ccstrong \\
        python full_catalog_pipeline/plot_margin_strain_profile.py --array T2
"""
...
from plot_margin_oriented_sections import (
    RELOC_COLS, SMOOTH_KM, PIX_M, SMOOTH_SIGMA_PX,
    ORIENT_RADIUS_KM, margin_axes, style, paths, INK, MUTED, GRID, SURF,
)
...
def main():
    ...
    ap.add_argument("--swath-km", type=float, default=SWATH_KM)
    args = ap.parse_args()
    P = paths(args.array)
    if not P["vx"]:
        raise SystemExit(f"no ITS_LIVE cache for {args.array}")
    ICE_BED_KM = P["ice_bed_km"]
    VX_CACHE, VY_CACHE = P["vx"], P["vy"]
    OUT_DIR = P["out_dir"]
    swath_km = args.swath_km
    OUT_PNG = os.path.join(OUT_DIR,
                           f"{args.array.lower()}_margin_strain_seismicity_profile.png")
    print(f"{args.array}: reloc {P['reloc']}  ice-bed {ICE_BED_KM} km")
    ...
    fig.suptitle(f"{args.array}: does seismicity track surface strain across the margin?",
    ...
    csv = os.path.join(OUT_DIR,
                       f"{args.array.lower()}_margin_strain_seismicity_profile.csv")
